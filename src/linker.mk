...
linker_script: $(STATIC_FILES) $(LINKED_FILES) $(SHARED_FILES)
	...
	$(Q)$(call APPEND,precious .header$(comma) .icon$(comma) .launcher if .libs.length$(comma) .libs$(comma) .start$(comma) .init_array$(comma) .ctors$(comma) .dtors$(comma) .fini_array)
