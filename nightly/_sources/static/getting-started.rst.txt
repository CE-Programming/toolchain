...
If you would rather try the "nightly" build (which includes the latest features and bug fixes before an official release), you can find it `here <https://github.com/CE-Programming/toolchain/releases/tag/nightly>`__.
...
**On Windows**
    ...
    * Double-click :code:`cedev.bat` inside the extracted *CEdev* folder to launch the toolchain environment.
      You may have to allow the script to be run depending on User Account Control.
...
**On Linux or macOS**
    ...
    * Extend the PATH environment variable to point to :code:`CEdev/bin` by modifying either the :code:`~/.zshrc` or :code:`~/.bashrc` in your home directory.
      An example command that can be added looks similar to the following:
...
.. tip::
    ...
    The toolchain update process is the same as installation; the only extra step is to delete the current ``CEdev`` folder if it exists beforehand.
...
library_examples
  ...
  They are used by many existing projects - the libraries themselves are downloadable `here <https://github.com/CE-Programming/libraries/releases/latest>`_.
...
This section describes the standard flow for building CE toolchain projects, starting with the classic "Hello World" example.
...
.. note::
    ...
    The download can be found `at this link <https://support.microsoft.com/en-us/topic/the-latest-supported-visual-c-downloads-2647da03-1eea-4433-9aff-95f26a218cc0>`_; depending on the architecture, you will need to install either *vc_redist.x86.exe* or *vc_redist.x64.exe*.
...
autotest.json
  ...
  This feature may be useful for creating testable code and coverage.
