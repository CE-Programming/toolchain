...
At the time, most homes did not have any kind of computer (or tablet or smartphone), and computers cost thousands of dollars;
the TI-81 cost just $110 and students could learn programming on a portable, personal device.

Since 1990, TI has continued to improve its offerings incrementally, adding more functionality to the TI-81's original software.
...
Until the TI-84 Plus CE, the hardware of the TI-8x series was improved only incrementally.
...
Running code directly from flash memory instead of copying it to RAM first is known as XIP (execute-in-place) flash.
...
The original Z80 could only directly access 64K of memory, making access to additional memory awkward and slow.
The TI-81 has just 64K of ROM and 8K of RAM, while the TI-84 Plus C SE had 4 MB of flash and 128K of RAM, and getting access to everything was complicated.
...
This is an unusual size, and it also results in the toolchain having 24-bit :code:`int`.
...
"Common Size" is the size in bits/bytes of the type that Windows uses, as well as many other platforms;
...
The circuits that modern CPUs use to handle such types are large and complicated, and simply weren't feasible in the 70s when the Z80 was designed.
Therefore, software has to implement that functionality.

Like the Z80, the eZ80 also lacks native support for floating-point arithmetic, so :code:`float` is slower than :code:`int`.
...
However, IEEE floats have limited precision and have unintuitive rounding behavior.
...
Unfortunately, it is also much slower.
...
This means that reading from or writing to special addresses (pointers) won't actually access memory, but instead send a command or query to a hardware device.

For example, there is no RAM or flash at the address :code:`0xF60024`,
but if you write to that address, the LCD backlight controller will know you're trying to talk to it and change the LCD's brightness.
...
While you can freely read all of flash, the calculator's hardware prevents you from writing to flash, and in fact, attempting to do so will cause a crash.
...
- 8-bit palettized color, using one byte per pixel and allowing double buffering;
...
You'll even have to write your own routines if you want to use 16-bit color mode.
...
Fundamentally, the issue is that the eZ80, as a successor to the Z80, is designed to be used with low-speed, zero-latency memory.
...
Yes, the CPU literally runs as though it's only clocked at around 6 MHz (equivalent to a Z80 at around 50 MHz).
