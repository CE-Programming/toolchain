...
The OS comes with an implementation of ANSI C89 `sprintf`, which can reduce the size of a program by ~8 KiB. However, it is limited in functionality. It does not support length modifiers and won't accept :code:`long` or :code:`float` arguments.
...
`boot_asprintf` functions similarly to `asprintf <https://www.man7.org/linux/man-pages/man3/asprintf.3.html>`_. It automatically allocates a buffer containing the output from `boot_sprintf`. The buffer should be deallocated with :code:`free`. The returned buffer will be :code:`NULL` if an allocation or formatting error occurs.
