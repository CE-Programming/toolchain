...
Surprisingly, the TI-84 Plus CE is sold to students as a calculator and not a gaming console.
TI even implemented an entire highly accurate floating-point package.

TI's floating-point routines use binary-coded decimal, in which each hexadecimal nibble represents one decimal digit.
Each :code:`real_t` has 14 digits (7 bytes) in the significand, an 8-bit exponent that the OS limits to +/- 99, and a sign byte.
