...
void main(void) {
	...
	/* Tell the interrupt controller that the ON flag should latch and be enabled, as with the timer 1 interrupt*/
	int_LatchConfig = int_EnableConfig = INT_ON | INT_TIMER1;
...
void interrupt isr_timer1(void) {
	...
	memset_fast(lcd_vramArray, rand()%255, 320*240*2);
	
	/* Must acknowledge that the interrupt occured to clear the flag */
	int_Acknowledge = INT_TIMER1;
	/* Not technically required since the interrupt controller handles the signal */
